...
void vm_kerninit(void)
{
	...
	// Map the TIMER
    mapAddress(pagetable, PLIC_BASE + 0x201000, PLIC_BASE + 0x201000, PAGE_SIZE, PTE_R | PTE_W | PTE_A | PTE_D | PTE_SO);
    mapAddress(pagetable, 0x02050000, 0x02050000, PAGE_SIZE, PTE_R | PTE_W | PTE_A | PTE_D | PTE_SO);

    // Map the UART
    mapAddress(pagetable, UART_BASE, UART_BASE, PAGE_SIZE, PTE_R | PTE_W | PTE_A | PTE_D | PTE_SO);
    ...
    mapAddress(pagetable, (ulong)&_datas, (ulong)&_datas,
               ((ulong)memheap - (ulong)&_datas), PTE_R | PTE_W | PTE_A | PTE_D | PTE_SO);
