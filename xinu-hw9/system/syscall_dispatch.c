...
syscall sc_ptcreate(ulong *);
syscall sc_pthread_mutex_lock(ulong *);
syscall sc_pthread_mutex_unlock(ulong *);
...
/*
* TODO: 
* Create system calls for pthread_create, pthread_mutex_lock, and pthread_mutex_unlock
*   pthread_mutex_lock takes in an integer (which is the lock number).  The corresponding syscall calls lock_acquire
*   pthread_mutex_unlock takes in an integer (which is the lock number).  The corresponding syscall calls lock_release
*   pthread_create takes in four arguments (see ptcreate.c).  The corresponding syscall calls ptcreate
*   pthread_join doesn't need a syscall.  It can be implemented in user land.  It takes in two arugments the process to wait for and the retval. For our cases, retval is NOT used.
*/
